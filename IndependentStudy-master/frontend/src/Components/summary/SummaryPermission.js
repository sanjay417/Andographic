...
export default class SummaryPermission extends Component {
    ...
    constructor(props) {
        ...
        this.state = {
            ...
            bubbleSeries: [],
            drilldownData: [],
            drilldownseries: {series: [{type: "bubble", id: "", data: []}]},
            rotate: false
    ...
    populateDrillDownData = (pid) =>{

        const drilldownData = this.state.drilldownData;
        this.state.drilldownData.series[0].id = pid

        let series = [];

        for(let i = 0; i < drilldownData.length; i++){
            let data = drilldownData[i];
            this.state.drilldownData.series[0].data.push({name: data.genre, value: data.total});
            alert("distict");
        }
    ...
    plotGraph = () =>{
        ...
        for(i = 0; i < permissionCount.length; i++){
            ...
            bubbleSeries[0].data.push({name: name, value: permissionCount[i].total, drilldown: name});
        }
        series.push(element);
    ...
    populateDistinctGenre =(pid, vtdetections) =>{
        let url =  "/api/distinctGenreByPermission" + '?pid='+ pid+ '&vtdetections='+ vtdetections;
        axios.get(url).then((result)=>{
            this.setState({drilldownData: result.data});
        }).then(()=>{
            this. populateDrillDownData(pid);
        }). catch((err)=>{
            console.log(err)
        })
    ...
    populatePermissionUsage =(path1, pids, vtdetections )=>{

        if(this.state.index == "Malicious" && vtdetections.length == 0)
            return;

        this.setState({rotate : true})
        let url =  path1 + '?pids='+ pids.join() + '&vtdetections='+ vtdetections;
        axios.get(url).then((result)=>{
    ...
    drilldown =(name) =>{
        let pid = this.getPermissionId(name);
        this.populateDistinctGenre(pid);
        alert(name);
    }
    ...
    renderChart =()=>{
        ...
        const bubbleChartOptions = {
            title :  "Permission Usage",
            name: "Genre",
            drilldown: this.state.drilldownseries,
        ...
        switch(this.state.chartType ){
            ...
            case  "bubble" : return <BubbleChartDrill options = {bubbleChartOptions} onClick = {this.drilldown.bind(this)} />
        }
    ...
    render() {
        ...
        if(this.props.options.permissions && this.state.pids.length == 0){

            pids = this.setPids(this.props.options.permissions);
