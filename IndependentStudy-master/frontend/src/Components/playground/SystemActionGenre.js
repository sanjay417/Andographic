...
export default class SystemActionGenre extends Component {
   ...
   /* plotCategoryGraph = () =>{
    ...
    onFilterChange =(result, selectedVals) =>{
        ...
        this.repopulateCharts(filters, this.state.genreFilter, this.state.vtdetectionsFilter, measure, this.state.index);
    ...
    repopulateCharts(measureFilter, genreFilter, vtdetectionFilter, measure, index) {
        if(genreFilter == undefined || genreFilter.length == 0)
            return;
        if (measure === "PermissionAll" || measure === "Action") {
            measureFilter.join(",");
...
*/
    ...
    renderChart =()=>{
        ...
        const chartOptions = {
            title : "Number of System Actions by Application Genre",
