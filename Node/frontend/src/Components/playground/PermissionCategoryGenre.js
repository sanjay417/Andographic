...
export default class PermissionCategoryGenre extends Component {
    ...
    onFilterChange =(result, selectedVals) =>{
        ...
        this.repopulateCharts(filters, this.state.genreFilter, this.state.vtdetectionsFilter, "Category", this.state.index);
    ...
    repopulateCharts(measureFilter, genreFilter, vtdetectionFilter, measure, index) {
        ...
        if (measure === "Category") {
            ...
            this.populateCategoryPermissionCount(measureFilter, vtdetectionFilter.join(), index);
        } else {
    ...
    populateCategoryPermissionCount =(filters, vtdetections, index)=>{
        let data = [];
        ...
        for(let i = 0; i < filters.length; i++){
            for(let j = 0; j < genres.length; j++){
                ...
                axios.get('/api/getPermissionsByGenre/'+category+"?genres="+encodeURIComponent(genre) +"&vtdetections=" + vtdetections+"&count="+index).then((result)=>{
    ...
    renderChart =()=>{
        ...
        const chartOptions = {
            title : "Dangerous Permissions usage by Application Genre",
    ...
    render() {
        ...
        const dropDownOptions ={
            ...
            title: "Select Dangerous Permission Category"
        }
