...
export default class SystemActionDeveloper extends Component {
    ...
    plotGraph = () =>{
        ...
        const systemActionsCount = this.state.systemActionsCount;
        for(let i = 0; i < systemActionsCount.length; i++){
            let obj = systemActionsCount[i];
            let permTotal = Math.round(obj.actionTotal);
            xAxis.push(obj.DeveloperName);
            element.data.push(permTotal);
            pieElement.data.push({name: obj.DeveloperName, y: permTotal});
            bubbleSeries[0].data.push({name: obj.DeveloperName, value: permTotal});
        }
    ...
    renderChart =()=>{
        ...
        const chartOptions = {
            title : "Average System Action usage by Developer",
